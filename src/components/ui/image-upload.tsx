...
export function ImageUpload({
  ...
  const resolvedAccept =
    accept ?? (mode === "document" ? DOCUMENT_ACCEPT : IMAGE_ACCEPT);
  const uploadLabel =
    label ??
    (mode === "document"
  ...
  const uploadHint =
    mode === "document" ? "PDF, DOC, CSV, XLSX" : "PNG, JPG, WEBP";
  const fileName = value
    ? decodeURIComponent(value.split("/").pop()?.split("?")[0] ?? "file")
    : "";

  useEffect(() => {
    onUploadPendingChange?.(uploadMutation.isPending);
  }, [uploadMutation.isPending, onUploadPendingChange]);
    ...
    <div>
      {value ? (
        mode === "document" ? (
          ...
          <div
            className={`relative rounded-lg overflow-hidden border border-gray-200 bg-slate-50 ${previewHeight}`}
          >
            <img
              src={value}
              alt="Cover preview"
