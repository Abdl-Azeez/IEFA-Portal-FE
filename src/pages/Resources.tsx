...
const SUB_CATEGORIES: SubCategoryConfig[] = [
  ...
  { key: 'educational-guides', label: 'Educational Guides', icon: BookOpen, resourceType: 'guide' },
  { key: 'research-publications', label: 'Research & Publications', icon: FileText, resourceType: 'research' },
  { key: 'standards-governance', label: 'Standards & Governance', icon: Shield, resourceType: 'standard' },
  { key: 'tools-practical', label: 'Tools & Practical', icon: Wrench, resourceType: 'tool' },
...
const REGULATORY_BODIES: RegulatoryBody[] = [
  {
    ...
    color: '#006847',
    description: 'Nigeria\'s apex monetary authority responsible for financial system stability.',
  },
  {
    id: 'sec',
    name: 'SEC',
    ...
    color: '#003082',
    description: 'Regulator of the Nigerian capital market and investment activities.',
  },
  {
    id: 'naicom',
    name: 'NAICOM',
    ...
    color: '#8B0000',
    description: 'Supervisory and regulatory authority for the insurance industry.',
  },
  {
    id: 'ndic',
    name: 'NDIC',
    ...
    color: '#1a4b8c',
    description: 'Protects depositors and maintains confidence in the banking system.',
  },
]
...
const DOCUMENT_TYPES: DocumentTypeConfig[] = [
  {
    ...
    description: 'Actionable, binding instructions with immediate compliance deadlines.',
  },
  {
    id: 'guidelines-frameworks',
    label: 'Guidelines & Frameworks',
    description: 'Comprehensive operational manuals (e.g., Basel III, Open Banking framework).',
  },
  {
  ...
  },
  {
    id: 'communiques',
    label: 'Communiques',
    description: 'Outcomes of meetings (e.g., Monetary Policy Committee decisions).',
  },
]
...
export default function Resources() {
  ...
  useEffect(() => {
    ...
    if (
      sub &&
      (
        ['all', 'educational-guides', 'research-publications', 'standards-governance', 'tools-practical', 'glossary'] as string[]
      ).includes(sub)
    ) {
  ...
  const subCategoryCounts = useMemo(() => {
    ...
    return counts
  }, [publishedResources, glossaryData?.length, activeTopCategory])

  /* ── Top-level category counts ── */
  const topCategoryCounts = useMemo(() => {
    const total = publishedResources.length
    return {
      all:        total,
  ...
  const currentResources = useMemo(() => {
    ...
    else if (sortBy === 'downloads') items.sort((a, b) => b.downloadCount - a.downloadCount)
    else
      items.sort(
        (a, b) =>
    ...
    return items
  }, [resourcesData?.data, sortBy, activeTopCategory])

  const apiCategoryChips = useMemo(
    () => apiSubCategories.map(c => ({ id: c.id, name: c.name })).sort((a, b) => a.name.localeCompare(b.name)),
    [apiSubCategories],
  )
  ...
  async function handleSubmitResource() {
    if (!uploadForm.title.trim() || !uploadForm.categoryId || !resourceFile) return
    try {
      setUploading(true)
      const fileUrl = await uploadFile.mutateAsync(resourceFile)
      let coverImageUrl: string | undefined
      if (coverFile) coverImageUrl = await uploadFile.mutateAsync(coverFile)
      ...
      await submitResource.mutateAsync({
        ...
        resourceType: (uploadForm.resourceType as import('@/types/resources').ResourceType) || undefined,
        subCategoryId: isCustomSub ? undefined : (uploadForm.subCategoryId || undefined),
        suggestedSubCategoryName: isCustomSub ? (uploadForm.customSubCategory.trim() || undefined) : undefined,
        suggestedDocTypeName: isCustomDoc ? (uploadForm.customDocType.trim() || undefined) : undefined,
        fileUrl,
        coverImageUrl,
        authorName: uploadForm.authorName || undefined,
      })
      toast({ title: 'Resource submitted!', description: 'Your resource is pending admin review.' })
      setUploadOpen(false)
  ...
  const isLoading = activeSubCategory === 'glossary' ? glossaryLoading : resourcesLoading
  const totalCount = publishedResources.length + (glossaryData?.length ?? 0)

  const uploadSubCats = uploadForm.categoryId === 'general' ? apiSubCategories : []
  ...
  return (
    <>
      ...
      >
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center gap-8 w-full">
              <div className="flex-1 space-y-4">
                ...
                <h1 className="text-3xl md:text-4xl font-bold text-white leading-tight">
                  Your gateway to{' '}
                  <span className="text-[#D52B1E]">Islamic Finance</span> knowledge
                </h1>
                <p className="text-gray-400 text-sm md:text-base leading-relaxed max-w-2xl">
                  Curated guides, research publications, regulatory standards, and practical tools
                  designed to help you learn about Islamic finance.
                </p>
                ...
                <div className="flex gap-3 pt-1 flex-wrap">
                  {[
                    ...
                    { label: 'Glossary Terms', value: glossaryData?.length ?? 0 },
                  ].map(stat => (
                    <div
                      key={stat.label}
        ...
        >
          <div className="flex items-center justify-between gap-2 overflow-x-auto scrollbar-hide py-3 px-4">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold bg-[#D52B1E]/10 text-[#D52B1E] border border-[#D52B1E]/20 hover:bg-[#D52B1E]/20 transition-all shrink-0"
            >
            ...
            <AnimatePresence mode="wait">
              {activeRegBody ? (
                ...
                >
                  {/* Back + body header */}
                  {(() => {
                    const body = REGULATORY_BODIES.find(b => b.id === activeRegBody)!
                    return (
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <img
                              ...
                              onError={e => {
                                ...
                                if (p && !p.querySelector('.reg-abbr-sm')) {
                                  ...
                                  s.className = 'reg-abbr-sm text-xs font-black'
                          ...
                          <div>
                            <p className="font-bold text-gray-900 text-sm">{body.name}</p>
                            <p className="text-xs text-gray-500">{body.fullName}</p>
                          </div>
                        </div>
                      </div>
                    )
                  ...
                  {activeDocType !== 'all' && (() => {
                    const dt = DOCUMENT_TYPES.find(d => d.id === activeDocType)
                    return dt ? (
                      <p className="text-xs text-gray-500 italic">{dt.description}</p>
                    ) : null
                  })()}
                  ...
                  <div className="py-16 text-center bg-white rounded-2xl border border-gray-100">
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      {REGULATORY_BODIES.find(b => b.id === activeRegBody)?.name} publications will appear here once uploaded.
                    </p>
                  </div>
                </motion.div>
              ) : (
                ...
                >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {REGULATORY_BODIES.map(body => (
                      <button
                        key={body.id}
                        onClick={() => { setActiveRegBody(body.id); setActiveDocType('all') }}
                      ...
                      >
                        ...
                        >
                          <img
                            ...
                            onError={e => {
                              ...
                              if (parent && !parent.querySelector('.reg-abbr')) {
                        ...
                        <div>
                          <p className="font-bold text-gray-900 text-base">{body.name}</p>
                          <p className="text-xs text-gray-500 mt-0.5 leading-snug">{body.fullName}</p>
                        </div>
                        <p className="text-xs text-gray-400 leading-relaxed">{body.description}</p>
                        <span
                        ...
                        {[
                          ...
                          >
                            ...
                            <div>
                              ...
                              <p className="text-xs text-[#737692] leading-relaxed">{tool.desc}</p>
                            </div>
                            {tool.action ? (
                              <a
                                href={tool.action}
                      ...
                      {visibleCount < currentResources.length && (
                        <div className="mt-8 flex flex-col items-center gap-2">
                          ...
                          <p className="text-xs text-gray-400">
                            Showing {Math.min(visibleCount, currentResources.length)} of{' '}
      ...
      >
        <div className="space-y-4">
          <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 text-xs text-amber-800">
            Your submission will be reviewed by our team before being published. We aim to respond
            within 2–3 business days.
          </div>

          <div>
            <label htmlFor="upload-title" className="block text-xs font-medium text-slate-600 mb-1">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label htmlFor="upload-category" className="block text-xs font-medium text-slate-600 mb-1">
              ...
              <Select
                ...
                onChange={e =>
                  setUploadForm(f => ({ ...f, categoryId: e.target.value, resourceType: '', subCategoryId: '', docTypeId: '' }))
                }
              >
            ...
            <div>
              <label htmlFor="upload-sub-category" className="block text-xs font-medium text-slate-600 mb-1">Sub-Category</label>
              <Select
                id="upload-sub-category"
                value={uploadForm.subCategoryId}
                onChange={e => setUploadForm(f => ({ ...f, subCategoryId: e.target.value, customSubCategory: '', docTypeId: '' }))}
              ...
              {/* Custom sub-category input — General only */}
              {uploadForm.categoryId !== 'regulatory' && uploadForm.subCategoryId === '__custom__' && (
                <div className="mt-2">
                  <Input
                    id="upload-custom-sub"
                    value={uploadForm.customSubCategory}
                    onChange={e => setUploadForm(f => ({ ...f, customSubCategory: e.target.value }))}
                    placeholder="Enter your sub-category name…"
                    className="h-9 text-sm"
                    autoFocus
                  />
                  <p className="text-xs text-amber-600 mt-1 flex items-center gap-1">
                    <span>⚠</span> This name will be reviewed and approved by an admin before publishing.
                  </p>
                </div>
              )}
            </div>
          ...
          {/* Resource Type — General only */}
          {uploadForm.categoryId === 'general' && (
            <div>
              <label htmlFor="upload-resource-type" className="block text-xs font-medium text-slate-600 mb-1">
                Resource Type
              </label>
              <Select
                id="upload-resource-type"
                value={uploadForm.resourceType}
                onChange={e => setUploadForm(f => ({ ...f, resourceType: e.target.value }))}
              >
          ...
          {uploadForm.categoryId === 'regulatory' && uploadForm.subCategoryId && (
            <div>
              <label htmlFor="upload-doc-type" className="block text-xs font-medium text-slate-600 mb-1">
                Document Type <span className="text-red-500">*</span>
              </label>
              <Select
                id="upload-doc-type"
                value={uploadForm.docTypeId}
                onChange={e => setUploadForm(f => ({ ...f, docTypeId: e.target.value, customDocType: '' }))}
              >
              ...
              {uploadForm.docTypeId === '__custom__' && (
                <div className="mt-2">
                  <Input
                    id="upload-custom-doc-type"
                    value={uploadForm.customDocType}
                    onChange={e => setUploadForm(f => ({ ...f, customDocType: e.target.value }))}
                    placeholder="Enter document type name…"
                    className="h-9 text-sm"
                    autoFocus
                  />
                  <p className="text-xs text-amber-600 mt-1 flex items-center gap-1">
                    <span>⚠</span> This document type will be reviewed and approved by an admin before publishing.
                  </p>
                </div>
              )}
            </div>
          ...
          <div>
            <label htmlFor="upload-author" className="block text-xs font-medium text-slate-600 mb-1">
          ...
          <div>
            <label htmlFor="upload-desc" className="block text-xs font-medium text-slate-600 mb-1">
              Brief Description
            </label>
            <textarea
              id="upload-desc"
              value={uploadForm.briefIntro}
              onChange={e => setUploadForm(f => ({ ...f, briefIntro: e.target.value }))}
            ...
            <div>
              ...
              <label className="flex items-center gap-2 cursor-pointer w-full h-9 text-sm border border-gray-200 rounded-lg px-3 hover:bg-gray-50 transition-colors">
                ...
                <span className="truncate text-gray-500">
                  {resourceFile ? resourceFile.name : 'Choose PDF / Doc…'}
                </span>
              </label>
            </div>
            <div>
          ...
          <div className="flex justify-end gap-3 pt-1">
            ...
            <Button
              ...
              disabled={!uploadForm.title.trim() || !uploadForm.categoryId || !resourceFile || uploading}
